...
Seguindo da aula passada para associar um consumidor a mais de um mangá, basta fazer que no *Map* ao invés de um mangá, seja uma lista de mangá, `<Consumer, List<Manga>>`. Também criaremos listas para associar:
...
Para iterar também utilizaremos o `Entry`, mas também será necessário iterar sobre o valor porque ele representa uma lista, desta forma conseguimos capturar o nome do mangá:
...
O `NavigableMap` é bem semelhante ao `NavigableSet`. No `NavigableSet`, é obrigatório que a classe que esteja sendo passada implemente *Comparable* ou então passamos um *Comparator*. Com o `NavigableMap` isto é necessário **SOMENTE**  para a chave, não o valor.

Um método é o `headMap(item, Boolean)`. Quando passamos o item, ele procura por outro mais baixo. O Booleano representa se queremos incluir o item que passamos.
...
public class NavigableMapTest01 {
    public static void main(String[] args) {
		...
		// Mesmos que tem no NavigableSet
